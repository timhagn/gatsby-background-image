...
export const createNoScriptImages = props => {
  if (Array.isArray(props.image)) {
    ...
    return props.image.reverse().map((data, index) => {
        if (isString(data)) {
          return `<div style="position:absolute;top:0;left:0;z-index:-${zIndex - index};opacity:${opacity};width:100%;height:100%;background:${data}"></div>`
        }
